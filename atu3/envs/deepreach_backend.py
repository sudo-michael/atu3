...
class DeepReachBackend:
    ...
    def __init__(self, experiment_dir="atu_multivehiclecollisionavoidtube_3") -> None:
        ...
        deepreach_verifcation_path = os.path.join(
            os.path.dirname(__file__), "../deepreach_verification/runs/"
        )

        model_path = os.path.join(
            deepreach_verifcation_path,
            experiment_dir,
            "training",
            "checkpoints",
            "model_final.pth",
    ...
    def ham(self, state, action):
        ...
        ham = self.dynamics.agent_hamiltonian(
            state[:, 1:].cuda(), dvs[..., 1:].cuda(), action
        )
    ...
    def plot(self):
        ...
        for i in range(len(times)):
            for j in range(len(zs)):
                coords = torch.zeros(
                    self.x_resolution * self.y_resolution, self.dynamics.state_dim + 1
                )
                coords[:, 0] = times[i]
                ...
                ax.set_title(
                    ...
                    % (
                        times[i],
                        plot_config["state_labels"][plot_config["z_axis_idx"]],
                        zs[j],
                    )
                )
                s = ax.imshow(
                    ...
                    * (
                        ...
                        .cpu()
                        .numpy()
                        .reshape(self.x_resolution, self.y_resolution)
                        .T
                        <= 0
                    ),
                    cmap="bwr",
