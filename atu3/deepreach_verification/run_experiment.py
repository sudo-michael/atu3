...
p.add_argument('--mode', type=str, required=True, choices=['all', 'train', 'test', 'plot'], help="Experiment mode to run (new experiments must choose 'all' or 'train').")

# save/load directory options
p.add_argument('--experiment_dir', type=str, default='./runs', help='Where to save the experiment subdirectory.')
...
if (mode == 'all') or (mode == 'train') or mode == 'plot':
    ...
    for param in dynamics_params.keys():
        if param == 'diff_model':
            p.add_argument('--' + param, type=bool, default=False, help='special dynamics_class argument')
        elif dynamics_params[param].annotation is bool:
...
experiment_dir = os.path.join(opt.experiment_dir, opt.experiment_name)
...
if mode == 'plot':
    print('plot')
    experiment.plot(-1, opt.val_x_resolution, opt.val_y_resolution, opt.val_z_resolution, opt.val_time_resolution)