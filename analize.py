...
from st import StatisticsTools
import os
...
class Plotter:
    ...
    def get_datasets(
        self, logdir: str, condition: str | None = None
    ) -> list[DataFrame]:
        ...
        for root, _, files in os.walk(logdir):
            if "progress.csv" in files:
                ...
                try:
                    with open(os.path.join(root, "config.json"), encoding="utf-8") as f:
                        ...
                        if "exp_name" in config:
                            ...
                            if "hj-cost" in root:
                                suffix = "-HJ"
                            elif "ham-cost" in root:
                                suffix = "-HAM"
                            exp_name = config["algo"]  + suffix
                ...
                performance = (
                    "Metrics/TestEpRet"
                    if "Metrics/TestEpRet" in exp_data
                    else "Metrics/EpRet"
                )
                cost_performance = (
                    "Metrics/TestEpCost"
                    if "Metrics/TestEpCost" in exp_data
                    else "Metrics/EpCost"
                )
    ...
    def make_plots(
        ...
        data = self.get_all_datasets(all_logdirs, None, select, exclude)
...
#     st = StatisticsTools()
#     st.load_source(PATH)
#     st.draw_graph(parameter='algo', values=None, compare_num=1, cost_limit=None, show_image=True)

# basically I want ot do this
# for all dir in PATH:
# if 'hj / ham ' in name of dir:
# move it over to
