...
class GoalAir3dEnv(gym.Env):
    ...
    def __init__(self, fixed_goal, walls, version=1) -> None:
        ...
        if self.walls:
            self.observation_space = gym.spaces.Dict({
                'observation': Box(low=np.array([-4.5, -4.5, -1, -1, -4.5, -4.5]),  # rx ry cos(rth) sin(rth) rxg, ryg
                                   high=np.array([4.5, 4.5, 1, 1, 4.5, 4.5]),
                                   dtype=np.float32),
                'achieved_goal': Box(low=np.array([-4.5, -4.5]), high=np.array([4.5, 4.5]), dtype=np.float32),
                'desired_goal': Box(low=np.array([-4.5, -4.5]), high=np.array([4.5, 4.5]), dtype=np.float32)
            })
            # world
            self.world_width = 10
            ...
            self.observation_space = gym.spaces.Dict({
                'observation': Box(low=np.array([-10, -10, -1, -1, -10, -10]),  # rx ry cos(rth) sin(rth) rxg, ryg
                                   high=np.array([10, 10, 1, 1, 10, 10]),
                                   dtype=np.float32),
                'achieved_goal': Box(low=np.array([-10, -10]), high=np.array([10, 10]), dtype=np.float32),
                'desired_goal': Box(low=np.array([-10, -10]), high=np.array([10, 10]), dtype=np.float32)
            })
        ...
        self.goal_location = np.array([2, 2])
        self.goal_r = 0.6
    ...
    def reset(self, seed=None):
        ...
        # self.persuer_state = np.array([-1.2, 1.2, -np.pi/2])
        self.last_dist_to_goal = np.linalg.norm(self.evader_state[:2] - self.goal_location)
        # info = {
        ...
        # }
        # return self.get_obs(info['obs'], info['persuer'], info['goal']), info
        return self.get_obs(self.evader_state.copy(), self.persuer_state.copy(), self.goal_location.copy())
    ...
    def near_goal(self, evader_state, goal_state, tol=None):
        ...
        if tol == None:
            return (
                np.linalg.norm(evader_state[:2] - goal_state) <= self.goal_r
            )
        else:
            return (
                np.linalg.norm(evader_state[:2] - goal_state) <= tol
            )
    ...
    def get_obs(self, evader_state, persuer_state, goal):
        ...
        relative_goal = evader_state[:2] - goal
        return {'observation': np.concatenate((relative_state, relative_goal)), 'achieved_goal': evader_state[:2].copy(), 'desired_goal': goal}
