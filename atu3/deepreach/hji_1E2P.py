...
from atu3.deepreach.modules import SirenNet
from atu3.deepreach.dataio import create_dataset_sampler, xy_grid
from atu3.deepreach.hj_functions import initialize_hji_loss, initialize_train_state, jacobian
...
def get_args():
    ...
    p.add_argument('--epochs', type=int, default=150_000)
    p.add_argument('--epochs-till-checkpoint', type=int, default=2_000)
    p.add_argument('--pretrain-end', type=int, default=10_000)
    p.add_argument('--counter-end', type=int, default=100_000)
...
def load_deepreach(ckpt_dir):
    ckpt_dir = os.path.join("atu3/deepreach/logs/", ckpt_dir)
