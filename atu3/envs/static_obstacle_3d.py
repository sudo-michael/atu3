...
class StaticAir3dEnv(gym.Env):
    ...
    def __init__(self, use_hj=False) -> None:
        self.car = car_brt
        self.dt = 0.05
        self.use_hj = use_hj
        ...
        self.evader_state = np.array([1., 1., np.pi/4])
        ...
        self.brt = np.load(
            # os.path.join(dir_path, f"assets/brts/static_obstacle_brat.npy")
            # os.path.join(dir_path, f"assets/brts/static_obstacle_min_brt.npy")
    ...
    def step(self, action):
        ...
        if np.linalg.norm(self.evader_state[:2] - self.goal_location) < self.goal_r + self.car.r:
            done = True
            info["collision"] = "goal"
        elif np.linalg.norm(self.evader_state[:2] - self.obstacle_location) < self.obstacle_r + self.car.r:
            info["collision"] = "obstacle"
            info["cost"] = 1


        return (
            np.copy(self.get_obs(self.evader_state, self.goal_location)),
            reward,
            done,
            info,
        )
    ...
    def reset(self, seed=None):
        ...
        return self.get_obs(self.evader_state, self.goal_location)
    ...
    def get_obs(self, evader_state, goal):
        return np.concatenate((self.theta_to_cos_sin(evader_state), goal[:2]))
